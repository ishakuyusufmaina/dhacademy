...
class InlinefieldBlot extends Embed {
    ...
    static create(value) {
		...
		let index = model.length;
		let stdModel = getStdInputModel() || [];
		model[index] = stdModel[index] || "";
		...
		if (stdWork.submitted){
			...
			if (stdWork.marked){
				...
				let marking = stdWork.marks[index];
				node.classList.add(marking);
				
				
			}
		}
		if (stdModel.submitted){
...
class CheckboxBlot extends Embed {
   static create(value) {
	...
	let stdModel = getStdInputModel() || [];
	let stdWork = getStudentWork();
	model[index] = stdModel[index] || false;
	let checked = model[index];
	inp.checked = checked;
	node.dataset.index = index;
	inp.onchange = e=>{
		model[index] = inp.checked;
		//alert(model);
	...
	if (stdWork.submitted){
			...
			if (stdWork.marked){
				node.classList.add("marked");
				let marking = stdWork.marks[index];
				if (marking) node.classList.add("correct");
...
class RadiogroupBlot extends Embed {
   static create(value) {
	...
	let stdModel = getStdInputModel() || [];
	stdModel[index] ??={value}
	model[index] = stdModel[index] || null;
	let stdWork = getStudentWork();
	value.options.forEach((optCont, i)=>{
		...
		label.prepend(rad);
		rad.checked = i === model[index];
		rad.onchange = e=>{
			if (rad.checked) model[index] = i;
			//alert(model);
		}
		node.append(label);
		const input = rad;
...
class MultilineEmbed extends BlockEmbed {
  ...
  static create(value) {
	...
	let stdModel = getStdInputModel() || [];
	model[index] = stdModel[index] || "";
	textarea.value = model[index];
	 textarea.addEventListener("change", e=>{
		  model[index] = textarea.value;
