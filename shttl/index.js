...
function initializeMeeting() {
  ...
  meeting.localParticipant.on("stream-enabled", (stream) => {
    let audioElement = null; //createAudioElement(meeting.localParticipant.id);
    let isLocal = true;
    setTrack(stream, audioElement, meeting.localParticipant, isLocal);
  });
...
window.onerror = function(message, source, lineno, colno, error) {
  ...
  alert(message);
    return true; // Returning true stops the error from logging to the browser console
};
window.addEventListener('unhandledrejection', (event) => {
    console.error('Unhandled Promise Rejection:', {
        reason: event.reason, // Usually holds the actual Error object
        promise: event.promise
    });
    alert(event.reason);
    // Optional: prevent the default browser error logging
